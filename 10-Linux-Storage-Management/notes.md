...
2. **Partitioning the Disk:**
   ...
   | *(Enter, Enter, Enter)* | Accept the defaults for partition number, first sector, and last sector (uses all free space) |
   ...
   **Important:** until `w` is pressed, nothing is actually written. If something looks wrong at any point, exiting with `q` (quit) instead of `w` discards everything safely — no risk of leaving a half-configured partition table.
...
6. **Testing the fstab Entry Safely:**
   ...
   A typo or wrong UUID in `/etc/fstab` won't show up immediately — it only causes a problem **the next time the system boots**, when the kernel tries to mount everything listed in `fstab` automatically. If an entry is broken at that point, the system can drop into **emergency mode**, requiring manual intervention with the root password through a recovery console just to get back to a normal boot.
...
- **Field 2 (Mount Point Target):** `/mnt/test_storage` – Where the partition gets mounted.
...
| **`mount` / `umount`** | Runtime Linkage | `sudo mount -a` | Mounts or unmounts filesystems; `-a` mounts everything listed in `fstab` and is the safe way to test it before a reboot. |
