...
| **Telemetry Scope**| Returns only the plain machine name. | Returns a full telemetry block including OS details, Kernel version, Architecture, and Hardware Vendor. |

### 🔍 Architectural Observation (Rocky Linux Default)
...
Instead of processing raw data dumps (e.g., viewing entire system metadata files), production DevOps workflows require filtering clean, actionable metrics via standard streams (`stdin`, `stdout`).
...
1. **`cat`**: Streams the raw telemetry payload of the OS release details.
...
In the custom tracking script, `awk` was utilized to target block device metrics cleanly:
...
## 3. Storage Analysis Matrix: `df` vs `du`

Two separate binaries are utilized daily to prevent system-wide lockups due to low storage block availability.

* **`df -h` (Disk Free):** Scans mounted block filesystems globally, translating raw bytes into human-readable notation (G, M). Used for structural dashboard overviews.
* **`du -sh <path>` (Disk Usage):** Recursively traverses specific local directory nodes (e.g., `/var/log`) to compile precise space allocation metrics. Used to hunt down logs or cache configurations bloating system spaces.
