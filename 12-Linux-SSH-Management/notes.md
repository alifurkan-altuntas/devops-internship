...
**Note:** `ssh-copy-id` is a bash script and isn't available on Windows by default (no native bash). On Windows, the same result has to be done manually:
...
SSH actively checks the permissions on `.ssh` and `authorized_keys` before trusting them. If they're too open, SSH may silently refuse to use them and fall back to password auth (or reject the connection entirely):
...
The reasoning: `authorized_keys` is effectively a list of "who is allowed in." If anyone else could edit it, they could add their own key and gain access — so SSH refuses to honor a file it can't be sure is private.
...
To confirm the restriction is real (not just assumed), it's worth testing both directions:
...
If you have separate keys for different servers (e.g. one for a Rocky Linux box, one for Ubuntu), SSH won't automatically guess which one to use unless it's saved to the default path (`~/.ssh/id_ed25519`). For a key saved under a custom name, you have to specify it explicitly:
...
Running in verbose mode helps diagnose this:
...
Opens an interactive session for browsing and transferring files — useful when you need to look around before deciding what to move.
