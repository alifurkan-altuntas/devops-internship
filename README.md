...
_`rclone serve http` için cache ve güvenlik konularını derinlemesine inceledim. Cache olmadan 10 paralel istek 9 saniye sürdü — bir an dondu sandım. Cache açıkken aynı test 0.194 saniye sürdü, 46 kat fark. S3'e yeni dosya ekleyince tarayıcıda görünmedi — `--dir-cache-time 1h` yüzünden dizin listesi cache'deydi, `rclone rc vfs/forget` ile temizleyince göründü. Auth için `--user` ve `--pass` flag'leri log'a şifreyi açık yazıyordu, environment variable ile sakladım. Remote control ile (`--rc`) rclone'u durdurmadan yönetebiliyorum._

- **Görevler & Hedefler:**
  - VFS cache ile performans testi yapıldı (cache'li 0.194s vs cache'siz 9.033s).
  - Dir cache davranışı test edildi — `vfs/forget` ile manuel temizleme.
